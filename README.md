...
- **🃏 Flashcards** — 33 themed/frequency vocabulary decks (~780 words) with
...
- **🔀 Conjugation** — fill-in drills across present, preterite, imperfect, and
  future tenses for ~70 verbs, with an accent-button bar (á é í ó ú ñ ü).
  Regular verbs are generated by a conjugation engine; irregular/stem-changing
  verbs use hand-written tables.
- **🔊 Pronunciation** — tap to hear any word/form, via the browser's built-in
  speech (no API key, works offline).
...
Learning content lives in a few files:

- [`js/data.js`](js/data.js) — `VOCAB_DECKS` (themed decks), `IRREGULAR_VERBS`
  (hand-written conjugation tables), and `GRAMMAR_QUESTIONS`.
- [`js/vocab_extra.js`](js/vocab_extra.js) — high-frequency decks appended to
  `VOCAB_DECKS`. Add a deck or words (`{ es, en }`) here.
- [`js/conjugator.js`](js/conjugator.js) — the regular-verb conjugation engine
  plus `REGULAR_VERBS`. To add a regular verb, just add `["infinitive", "to …"]`
  — its tables are generated automatically. Stem-changing or irregular verbs go
  in `IRREGULAR_VERBS` instead.
...
js/vocab_core.js    Frequency core (verbs, ideas, society, colors…)
